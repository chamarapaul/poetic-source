...
const patterns: [RegExp, TokenType][] = [
  ...
  [
    /@(?:interface|implementation|protocol|end|property|autoreleasepool|class|selector|synthesize|dynamic|encode|synchronized|try|catch|finally|throw)\b/,
    'keyword',
  ],
  ...
  [
    /\b(?:NSInteger|NSUInteger|NSObject|NSString|NSArray|NSDictionary|NSNumber|BOOL|NSNotFound)\b/,
    'type',
  ],

  // Language keywords
  [
    /\b(?:if|else|return|self|void|int|unsigned|long|double|float|char|const|static|extern|typedef|struct|enum|class|protocol|id|nil|Nil|YES|NO|IBOutlet|IBAction|nonatomic|strong|weak|copy|assign|readonly|readwrite)\b/,
    'keyword',
  ],
